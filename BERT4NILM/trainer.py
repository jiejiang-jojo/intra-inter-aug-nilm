...
class Trainer(metaclass=ABCMeta):
    ...
    def train(self):
        ...
        logging.info('validate {}'.format(best_f1.mean() +best_acc.mean() - best_rel_err.mean()))
        self._save_state_dict(0,0)

        for epoch in range(self.num_epochs):
            best_f1,best_acc , best_rel_err=self.train_bert_one_epoch(epoch + 1,val_rel_err,val_abs_err,val_acc,val_precision,val_recall,val_f1,best_rel_err,best_acc,best_f1)
    ...
    def train_one_epoch(self, epoch):
        ...
        val_acc, val_precision, val_recall, val_f1 = [], [], [], []
        best_rel_err, _, best_acc, _, _, best_f1 = self.validate()
        logging.info('validate {}'.format(f1.mean() + acc.mean() - rel_err.mean()))
        ...
        for batch_idx, batch in enumerate(tqdm_dataloader):
            ...
            if iteration % 1000 == 0:
                logging.info('Epoch {}, loss {:.2f}'.format(epoch, average_loss))
                ...
                if f1.mean() + acc.mean() - rel_err.mean() > best_f1.mean() + best_acc.mean() - best_rel_err.mean():
                    ...
                    self._save_state_dict()
                    logging.info('best{}'.format(epoch+1))
        
        if self.args.enable_lr_schedule:
            self.lr_scheduler.step()
        return 
    def train_bert_one_epoch(self, epoch,val_rel_err,val_abs_err,val_acc,val_precision,val_recall,val_f1,best_rel_err,best_acc,best_f1):
        ...
        for batch_idx, batch in enumerate(tqdm_dataloader):
            ...
            if iteration % 1 == 0:
                ...
                if f1.mean() + acc.mean() - rel_err.mean() > best_f1.mean() + best_acc.mean() - best_rel_err.mean():
                    ...
                    best_acc = acc
                    best_rel_err = rel_err
                    self._save_state_dict(epoch,iteration)
                    logging.info('best{}'.format(epoch))
            iteration+=1
        if self.args.enable_lr_schedule:
            self.lr_scheduler.step()
        return best_f1,best_acc , best_rel_err
    def validate(self):
        
        self.model.eval()
        loss_values, relative_errors, absolute_errors = [], [], []
        acc_values, precision_values, recall_values, f1_values,  = [], [], [], []
        logging.basicConfig(filename='log/training_{}_3.log'.format(self.data_na), level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                rel_err, abs_err = validate_relative_absolute_error(logits_energy.detach(
    ...
    def test(self, test_loader,source):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                source_seqs.append(seqs.detach().cpu().numpy().tolist())
        label_curve = np.concatenate(label_curve).reshape(-1, self.args.output_size)
        print(len(label_curve),2)
        ...
        source_seqs=np.concatenate(source_seqs).reshape(-1, self.args.output_size)
        print(len(source_seqs),3)
        ...
        source=source.get_original_data(source_seqs)
        source=source[0].reshape(-1, self.args.output_size)
        
        valid_data = np.ones_like(source)
        for i in range(len(source)):
            if (source[i]==0) or (source[i] < label_curve[i]):
                valid_data[i] = 0
        
        np.save('output/aa_{}prediction_{}_{}.npy'.format(self.data_key,self.test_name,self.data_na),e_pred_curve)
        np.save('output/aa_{}groundtruth_{}_{}.npy'.format(self.data_key,self.test_name,self.data_na),label_curve)
        np.save("output/aa_{}source_{}_{}.npy".format(self.data_key,self.test_name,self.data_na),source)

        
       
        #return_rel_err, return_abs_err = relative_absolute_error(e_pred_curve,label_curve)
        mae=mean_absolute_error(e_pred_curve*valid_data, label_curve*valid_data)
        sae=signal_aggregate_error(e_pred_curve*valid_data, label_curve*valid_data)
        return_acc, return_precision, return_recall, return_f1 = acc_precision_recall_f1_score(s_pred_curve, status_curve)

        return sae, mae, return_acc, return_precision, return_recall, return_f1
        #return  mae, sae, return_acc, return_precision, return_recall, return_f1
    ...
    def _load_best_model(self):
        try:
            # print('best_acc_model{}_{}_{}.pth'.format(args.epo,args.iter,args.data_na))
            # self.model.load_state_dict(torch.load(
            #     self.export_root.joinpath('best_acc_model{}.pth'.format(self.data_na))))
            self.model.load_state_dict(torch.load(
                self.export_root.joinpath('best_acc_model{}_{}_{}.pth'.format(args.epo,args.iter,args.data_na))))
            
            self.model.to(self.device)
        except:
            print('Failed to load best model, continue testing with current model...')

    def _save_state_dict(self,epoch,iteration):
        if not os.path.exists(self.export_root):
            os.makedirs(self.export_root)
        print('Saving best model...')
        torch.save(self.model.state_dict(),
                   self.export_root.joinpath('best_acc_model{}_{}_{}.pth'.format(epoch,iteration,self.data_na)))
                #    self.export_root.joinpath('best_acc_model{}.pth'.format(self.data_na)))
