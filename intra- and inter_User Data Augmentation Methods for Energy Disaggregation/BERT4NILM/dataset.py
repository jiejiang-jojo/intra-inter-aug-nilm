...
class AbstractDataset(metaclass=ABCMeta):
    def __init__(self, args, stats=None):
        ...
        self.x, self.y = self.load_data(args.data_na)
        #self.x, self.y = self.load_data()
        
       
        self.origin_length = len(self.x)
        
        print('Origin length:', self.origin_length)
    ...
    def get_origin_length(self):
        return self.origin_length

    def get_data(self):
        return self.x, self.y, self.status

    def get_original_data(self,x):
        x_org = x * self.x_std + self.x_mean
...
class UK_DALE_Dataset(AbstractDataset):
    ...
    def load_data(self):
        ...
        #     raise FileNotFoundError
            ...
            directory = Path('')
            print(self.appliance_names)
            print(self.house_indicies)
            for house_id in self.house_indicies:
                house_folder = directory.joinpath('house_' + str(house_id))
                house_label = pd.read_csv(house_folder.joinpath(
                    'labels.dat'), sep=' ', header=None)
...
class REFIT_Dataset(AbstractDataset):
    ...
    def _if_data_exists(self):
        folder = Path(RAW_DATASET_ROOT_FOLDER).joinpath(self.code())
        first_file = folder.joinpath('house1.csv')
        if first_file.is_file():
            return True
        return False

    def load_data(self,data_na):
        
        print(self.appliance_names)
        print(self.house_indicies)
        if self.appliance_names[0] == 'electromobile':
            length, arr1, arr2 = self.load_ebike_data()
            return length, arr1, arr2
         
        for appliance in self.appliance_names:
            assert appliance in ['dishwasher', 'microwave', 'washingmachine', 'kettle', 'electromobile']
        ...
        #     raise FileNotFoundError
            ...
            hf = h5py.File('/home/shijie/nilm/fast-seq2point-master/{}.h5'.format(data_na), 'r')
 
            aggregates = []
            targets = []
            for house_id in self.house_indicies:
                house = 'house{}'.format(house_id)
                aggregate = hf[house]['aggregate'][:]
                target = hf[house][self.appliance_names[0]][:]
            ...
            entire_data = pd.DataFrame(aggregates, columns=['aggregate'])
            entire_data[self.appliance_names[0]] = targets
           
            length = len(entire_data.values[:, 0])
            
          
            # entire_data = entire_data.dropna().copy()
            # entire_data = entire_data[entire_data['aggregate'] > 0]
            # entire_data = entire_data[entire_data['aggregate'] > entire_data[self.appliance_names[0]]]
            # entire_data[entire_data < 5] = 0
            # entire_data = entire_data.clip(
            #     [0] * len(entire_data.columns), self.cutoff, axis=1)
        ...
        print(len(entire_data.values[:,0]),1)
     
        return np.array(entire_data.values[:,0].tolist()), np.array(entire_data.values[:, 1:].tolist())
